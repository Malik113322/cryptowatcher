...
const CoinDetails = () => {
  ...
  if (error) return <ErrorPage message={'Error while fetching coin'} />
  ...
  return (
    <Container maxW={'container.xl'}>

      {
        loading ? (<Loader />) : (
          <>
            <Box w={'full'} h={['50vh', '100vh']} borderWidth={1}>
            ...
            <HStack p='4' wrap={'wrap'}>
              {
                btns.map((i) => (
                  <Button key={i} onClick={() => switchChartStats(i)} my='2' mx={['2', '0']} p={['3', '5']}>{i}</Button>
                ))
              }
            </HStack>
            ...
            <VStack spacing={'4'} padding={'16'} alignItems={'flex-start'} >
              ...
              <Badge bgColor={'blackAlpha.500'} fontSize={'2xl'}>
                {`#${coin.market_cap_rank}`}
              </Badge>

              <CustomBar low={`${currencySymbol}${coin.market_data.low_24h[currency]}`}
...
const CustomBar = ({ low, high }) => (
  <VStack w={'full'}>
    <Progress value={50} bgColor={'teal'} w={'full'} />
